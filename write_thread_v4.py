...
def msgHandler(cmdQ, writeQ):
    ...
    AverageLatency = []
    Latency = np.array([])
    np.expand_dims(Latency, axis = 0)
    np.expand_dims(Latency, axis = 1)
    print(Latency.shape)
    #data Containers. Very ugly but functional
                ...
                for j in range(0,20):
                    ...
                    while reply == False:
                        ...
                        if(time.time()-START > 10):
                            break
                    dataTimes.append(time.time()-START)
                    print("Progress: " + str(j) + " | " + str(dataTimes[-1]))
                print("Average Latency: "+str(np.mean(dataTimes)))
                if( (Latency.shape)[0] == 0):
                    Latency = np.copy(np.array(dataTimes))
                    Latency = Latency[:,np.newaxis]
                    print(Latency.shape)
                else:
                    Latency = np.append(Latency, np.array(dataTimes)[:,np.newaxis], axis = 1)
                AverageLatency = np.append(AverageLatency, np.mean(dataTimes))
                print("sleeping...")
                time.sleep(60*5)
                print("Awake!")
            print("Saving results...")
            sio.savemat('latency_results.mat', {'Latency':Latency, 'Times':times, 'AverageLatency':AverageLatency})
