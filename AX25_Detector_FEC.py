...
class AX25Detector:
    ...
    def __init__(self):
        super().__init__()
        self.startlengthcheck = 5
        self.taillengthcheck = 1
        self.state = 0
        self.flagbuffer = collections.deque(maxlen = self.startlengthcheck*8)
        self.msgBuffer = []
        self.msgBuffer_raw = []
        self.bitCount = 0
        self.packetCount = 0
    ...
    def queBit_FEC(self, y):
        if self.state == 0:
            ...
            if compareBytes(bits2bytes(list(self.flagbuffer)), self.startlengthcheck*[int("0x7E", 16)]) <= self.startlengthcheck:
                #print("FLAG DETECTED")
                self.msgBuffer = []
                self.msgBuffer_raw = []
                self.state = 1
            ...
            self.msgBuffer.append(bit)
            self.bitCount = self.bitCount + 1
            if compareBytes(bits2bytes(list(self.msgBuffer[-self.taillengthcheck*8:])), self.taillengthcheck*[int("0x7E", 16)]) <= 0:
                ##end sequence detected!

                ##destuff msg:
                self.msgBuffer = AX25_Encoder.AX25Encoder.DeStuffBits(self.msgBuffer[:-8*self.taillengthcheck])
    ...
    def queBit_FEC_FIXED_LEN(self, y, LEN):
        if self.state == 0:
            ##Detect Flag
            inbit = soft2hardBit(y)
            bit = self.g3ruhEncoder.DescrambleBit(self.g3ruhEncoder.NRZIDecodeBit(inbit))
            self.flagbuffer.append(bit)
            if compareBytes(bits2bytes(list(self.flagbuffer)), self.startlengthcheck*[int("0x7E", 16)]) <= 0:
                #print("FLAG DETECTED")
                self.msgBuffer = []
                self.msgBuffer_raw = []
                self.state = 1
